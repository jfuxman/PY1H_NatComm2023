...
### Events
df = pd.read_excel('Final events list 6FEB23.xlsx')

tfs = df['TF1'].tolist() + df['TF2'].tolist()
tfs = list(set(tfs))

print(df)
print(tfs)
cytokines = list(set(df['DNA bait'].tolist()))
print(cytokines)
cytokines = [str(i) for i in cytokines]

# Do blast Cytokine sequence vs human genome to get coordinates
info = dict(list(zip(df['DNA bait'], df['DNA bait sequence'])))
...
for k,v in info.items():
	...
	o1.write('>' + str(k) + '\n' + v + '\n')
o1.close()
subprocess.call('blastn -db GRCh38.fa -query tmp.fasta -out blast.results.out -outfmt "6 qgi qacc sacc qlen qstart qend sstart send evalue length mismatch"', shell = True)
...
for cytokine in cytokines:
	...
	tmp_df = tmp_df[(tmp_df['cytokine'] == int(cytokine)) & (tmp_df['qend'] == seq_len) & (tmp_df['qstart'] <= 2)]
	...
	subdf = df[df['DNA bait'] == int(cytokine)]
...
results.to_excel('MACS2_Peaks_TFs_final_16_feb.xlsx')
